...
const SeasonEpisodes = () => {
  ...
  useEffect(() => {
    fetch(`https://podcast-api.netlify.app/id/${id}`)
      ...
      .then((showData) => {
        const selectedSeason = showData.seasons.find(
          (seasonData) => seasonData.season === parseInt(seasonNumber)
        );
  ...
  return (
    <div>
      ...
      <Link to={`/show/${id}`}><button className='button'>Back to Show Details</button></Link>
    </div>
  );
};
